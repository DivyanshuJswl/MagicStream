...
import (
	...
	"github.com/divyanshujswl/MagicStream/Server/MagicStreamServer/models"
	"github.com/divyanshujswl/MagicStream/Server/MagicStreamServer/utils"
...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		role, err := utils.GetRoleIdFromContext(c)
		if err != nil {
...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context) (string, int, error) {
	...
	for _, ranking := range rankings {
		if ranking.RankingValue != 999 {
			sentinmentDelimited = sentinmentDelimited + ranking.RankingName + ","
		}
	}

	sentinmentDelimited = strings.Trim(sentinmentDelimited, ",")
	...
	if GroqApiKey == "" {
		return "", 0, errors.New("Could not find GROQ_API_KEY")
	}

	clientGroq := groq.NewClient(groq.WithAPIKey(GroqApiKey))

	base_prompt_template := os.Getenv("BASE_PROMPT_TEMPLATE")

	base_prompt := strings.Replace(base_prompt_template, "{rankings}", sentinmentDelimited, 1)

	fullPrompt := base_prompt + "\n\nReview: " + admin_review

	response, err := clientGroq.CreateChatCompletion(groq.CompletionCreateParams{
		Model: "openai/gpt-oss-20b", // Use a model available on Groq
		...
		Messages: []groq.Message{
			{
				Role:    "system",
				Content: "You are a sentiment classifier. You must respond with ONLY one word from the provided list. Do not include any punctuation, explanation, or additional text.",
			},
			{
				Role:    "user",
				Content: fullPrompt,
...
func GetRankings(client *mongo.Client, c *gin.Context) ([]models.Ranking, error) {
	...
	var rankingCollection *mongo.Collection = database.OpenCollection("ranking", client)
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var recommendedMovieListVal int64 = 5
		recommendedMovieListStr := os.Getenv("RECOMMENDED_MOVIE_LIMIT")

		if recommendedMovieListStr != "" {
			recommendedMovieListVal, _ = strconv.ParseInt(recommendedMovieListStr, 10, 64)
		}

		findOptions := options.Find()
		findOptions.SetSort(bson.D{{Key: "ranking.ranking_value", Value: 1}})

		findOptions.SetLimit(recommendedMovieListVal)
...
func GetUsersFavoriteGenres(userId string, client *mongo.Client, c *gin.Context) ([]string, error) {
	...
	if !ok {
		return []string{}, errors.New("unable to retrieve favorite genres to user.")
	}
